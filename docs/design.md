...
| `v1.7.0` | `2026-02-23` | 完成 T13：定稿网关配置变更与回滚审计口径（历史记录）与发布包下载鉴权缓存策略，关闭 OQ-02/OQ-03。 | `T13`、`ADR-090` |
...
| `v1.27.0` | `2026-02-27` | 完成 T34：补齐部署视图 Redis 组件表达，收敛 `3.2/3.4/8.1.1` 执行底座口径并联动验收回标。 | `T34`、`ADR-110` |
| `v1.28.0` | `2026-02-27` | 完成 T35：统一 `user` 角色词义并清理“泛指用户”歧义文案，联动验收与覆盖回标。 | `T35`、`ADR-111` |
...
| `instance_id` | 历史双视图（T24）兼容标识，仅用于历史兼容窗口（`v1.20.0~v1.21.x`）的 `deprecated` 路径 `/instances/{instance_id}`；`v1.22.0` 起已退出现行外部契约，不得用于现行主接口参数。 |
...
- 网关流量治理策略算法（流量权重、步长放量、阈值回切）实现细节；Aether 仅约束配置变更、回滚与审计边界。
- 工作空间内按资源创建者（`owner_user_id`）做授权隔离；当前固定采用“按 workspace 授权，不按创建者隔离”。
- 新增第四类角色或按角色层级扩展授权模型；当前固定采用 `2.5` 定义的三角色模型（`super_admin/workspace_admin/user`）。
...
角色模型固定为三级：`super_admin`、`workspace_admin`、`user`。

- `super_admin`：可跨工作空间执行平台治理与部署治理动作（含工作空间绑定/解绑、仓库绑定回滚、模板导入、网关与低代码平台治理）。
- `workspace_admin`：仅可在 `managed_workspace` 范围内执行治理与资源运维动作，不可越权到未授权工作空间。
- `user`：仅可在已关联工作空间内执行高代码应用与记忆体 CRUD，并进入已开通低代码平台执行平台内应用 CRUD；禁止独立 CRUD 共享数据服务组件、禁止网关 CRUD、禁止工作空间绑定治理动作。

术语冻结规则（T35）：

- 文本中出现反引号标记的 `user` 时，仅表示三角色模型中的普通用户角色。
- 需表达泛指主体时，统一使用“调用方”或“工作空间成员”，禁止再使用“用户”作为泛称：
  - 调用方：发起 API 请求的认证主体（可为 `super_admin`、`workspace_admin`、`user` 或 `system`）。
  - 工作空间成员：与目标工作空间建立关联关系的成员集合，不等价于 `user` 角色。

`managed_workspace` 授权关系模型（T21）：

- 授权来源：平台管理面成员关系主数据（Aether 只读消费）。
- 授权快照：登录态注入 `managed_workspace_ids`，供 API 准入与 Worker 二次鉴权复用。
- 刷新机制：令牌刷新、`membership.changed.v1` 事件消费、任务执行前二次拉取三条路径共同生效，取“最严格权限”。
- 审计最小字段：`actor_role`、`actor_id`、`workspace_id`、`resource_kind`、`resource_id`、`action`、`result`、`request_id`。
...
- `/api/v1/instances/*` 仅在历史兼容窗口（`v1.20.0~v1.21.x`）保留并标注 `deprecated: true`；
  `v1.22.0` 起已从现行外部契约移除，仅允许作为历史回放样例保留（见 7.5 OpenAPI 约束）。
...
- `MEMORY` 并入约束（T27）：`mem0` 不再作为数据服务组件类型参与共享组件抽象，
  其相关编排语义统一归入 `MemoryInstance` 与 `memory_component_refs`。
...
- 控制面执行底座组件固定为 `Postgres + Redis`：`Postgres` 承载事实源与 Outbox，`Redis` 承载队列、分布式锁与 EventBus 运行态。
...
- `R-ENV-002`：工作空间创建及其与集群、仓库关联由平台管理面负责；Aether 仅接受平台内部契约调用并消费生效绑定。
  ...
  工作空间成员管理与空间管理员配置同属平台管理面动作；
  Aether 仅消费生效成员关系用于 `managed_workspace` 与 `workspace_membership` 鉴权判定。
  关联入口固定为内部契约：
...
- `R-ENV-005`：创建资源时必须先确定 `workspace_id/cluster_id` 作用域；
  作用域参数可通过 query/body/path 承载，但不得改变 `applications-only` 主路径形态；`namespace` 由绑定表反查注入，不接受请求体覆写。
...
- `R-ENV-010`：解绑仅 `super_admin` 或该工作空间 `workspace_admin` 可发起，且必须二次确认（请求体 `confirmation_token`）；全过程写审计事件。
...
- 来源：平台管理面维护 `workspace_admin -> managed_workspace` 映射，Aether 仅消费只读快照。
- 生效：API 准入先校验 `workspace_id in managed_workspace_ids`，Worker 执行前再次校验，避免排队期间权限漂移导致越权。
- 失效：当授权被移除时，新请求立即拒绝；已入队任务在执行前被拒绝并写审计 `FORBIDDEN_ACTION`。
- 审计：绑定治理动作必须补齐 `actor_role/actor_id/workspace_id/binding_id/action/result/request_id`。
...
| 校验项 | 约束 | 失败码 |
| --- | --- | --- |
| 权限 | 仅 `super_admin` 或该工作空间 `workspace_admin` 可调用，且必须携带 `confirmation_token` | `403 FORBIDDEN_ACTION` / `422 CONFIRMATION_TOKEN_INVALID` |
...
- 统计对象：仅统计 6 类一级资源：
  ...
  `GatewayInstance`、`HighCodeApplication`、`MemoryInstance`。
...
### 4.3 数据服务组件（R-DSP-001~013）

资源域范围：

- 固定类型：`Postgres`、`Redis`、`milvus`、`weaviate`、`pgvector`、`NebulaGraph`、`MongoDB`。
- 模板来源：全部平台内置 Helm，按 `template_version` 可升级。
- `mem0` 边界迁移（T27）：
  - `mem0` 不再作为共享数据服务组件类型出现在创建入口与共享列表；
  - `mem0` 相关部署能力纳入 `MEMORY` 一级资源；
  - 历史存量若仍为 `embedded` 数据服务实例，仅允许由其宿主链路运维或回收，不得转为 shared。

实例分类与接口边界：

- `shared`：外部接口统一走 `/api/v1/applications`（`resource_type=DATA_SERVICE`），
  进入共享列表与选择器。
- `embedded`：仅随宿主管理，不提供独立创建入口，不进入共享列表；仅允许
  `super_admin` 通过 `{scope}/embeddeddataservices/{embedded_dataservice_id}` 执行独立运维。
- `embedded` 必须携带 `owner_kind/owner_id`，
  且 `owner_kind` 仅允许 `lowcode_platform`、
  `highcode_application` 或 `memory_instance`。
- `/api/v1/workspaces/{workspace_id}/clusters/{cluster_id}/dataservices*` 不再作为现行外部
  canonical path；若需历史回放，必须进入兼容包并显式标注历史基线。
...
- 冲突优先级 `P1 > P2 > P3`（与 `9.4` 一致）：
  ...
  - `P2`：shared 组件独立创建且显式传入 `service_name`，使用调用方提交名称（同 namespace 唯一）。
...
- 记忆体自动创建组件标记 `embedded_by_memory`，并固定 `owner_kind=memory_instance`。
- 嵌入式组件禁止被其他宿主复用绑定（高代码/低代码/记忆体均禁止跨宿主复用）。
- 共享组件与嵌入式组件必须在 API、控制台列表、权限校验中完全隔离。
- 删除宿主时嵌入式组件按宿主生命周期回收；失败进入补偿任务。
- 若 `cascade=true` 删除路径命中 shared 组件外部引用，统一返回 `409 REFERENCE_CONFLICT` 并阻断该组件删除。
...
- 开通/更新/删除（内置低代码平台）允许 `super_admin` 与 `workspace_admin(managed_workspace)` 执行（`R-LCP-002`）。
- 导入 `POST /lowcodes/imports` 仅 `super_admin` 可调用（`R-LCP-003`）。
- 用户仅可查看状态与入口 URL，并通过低代码平台入口执行平台内应用 CRUD（`R-LCP-009`、`R-LCP-010`）。
- 用户禁止独立开通/关闭低代码平台，触发时返回 `403 FORBIDDEN_ACTION` 并审计。
...
- 配置范围与变更策略：
  ...
  - 变更权限：`super_admin` 或 `workspace_admin(managed_workspace)` 可修改。
...
- `admin_account_ref` 仅返回引用值，不返回明文凭证；用户读取时按策略脱敏为
...
- 配置变更固定采用“变更请求 -> 校验 -> 生效 -> 审计”的任务化流程；执行失败必须自动回滚到最近稳定 `config_revision`（`R-GTW-006`）。
- 权限：`super_admin` 与 `workspace_admin(managed_workspace)` 可执行网关 CRUD；用户执行网关 CRUD 一律 `403 FORBIDDEN_ACTION`。
...
- 回滚动作固定回到上一个稳定 `config_revision`，并记录 `rollback_to_revision`。
- 审计字段最小集合：
...
- 演练至少覆盖一次“配置校验失败拦截”与一次“生效失败自动回滚”。
- 演练证据必须包含 `from/to revision`、回滚目标 revision、任务状态轨迹、审计事件 ID。
...
- `super_admin`：可跨工作空间执行高代码应用全量动作。
- `workspace_admin`：仅可在 `managed_workspace` 范围执行高代码应用全量动作。
- `user`：仅可在已关联 workspace 执行高代码应用 CRUD；禁止跨 workspace/cluster/namespace。
- 同一 workspace 内不按创建者隔离：不引入 `owner_user_id` 级授权门槛；鉴权仅校验 workspace 关联与角色动作许可。
...
### 4.9 应用关系持久化与资源可见性（R-DATA-001~009、R-DATA-010/011、R-MEM-001~008）
...
- 记忆体保存 `memory_main_instance + memory_component_refs[]`（`R-DATA-003`、`R-MEM-002`）：
  - `memory_main_instance` 记录主实例 `component_id/component_name/status/version/entry_url`；
  - `memory_component_refs[]` 记录 `shared|embedded`、`owner_kind/owner_id`、`health_snapshot`、`last_task_id`；
  - `owner_kind` 固定允许 `memory_instance`，并保留共享组件 `owner_kind=null`。
- 关系查询输出必须带 `visibility` 与 `owner_kind/owner_id`（`R-DATA-004`）。

查询与展示规则：

- Query 路径不做跨资源实时聚合状态计算（`R-DATA-005`）。
- 应用详情固定展示：Agent 集合、组件集合、入口地址、发布 Chart 版本（`R-DATA-008`）。
- 共享选择器仅展示 `visibility=shared` 且 `deleted_at is null`（`R-DATA-007`）。
- 记忆体创建来源限制（`R-MEM-003`）：仅允许平台内置 Helm 模板，不接受镜像上传、外部 Helm 导入或用户自定义模板源。
- 记忆体关联限制（`R-MEM-005`）：主实例与内部组件必须同 `workspace_id + cluster_id + namespace`，任一跨边界关联统一 `422 SCHEMA_VALIDATION_FAILED`。
- 记忆体运维视图最小字段（`R-MEM-008`）：`version`、`entry_url`、`dependency_topology`、`last_orchestration_task`。

删除与审计：

- 软删字段：`deleted_at`、`deleted_by`、`delete_task_id`（`R-DATA-006`）。
- 绑定/解绑/级联删除均写审计事件（`R-DATA-009`）。
- 记忆体删除语义（`R-MEM-006`）：
  - `cascade=false`：仅删除记忆体主实例，保留共享子实例；
  - `cascade=true`：允许删除嵌入式/独占子实例；若共享子实例仍被其他宿主引用，返回 `409 REFERENCE_CONFLICT` 并阻断该实例删除；
  - 失败分支进入补偿任务并记录 `memory_component_reconcile` 审计。
...
- 返回分支规则（按 `resource_type`）：
  ...
  - `MEMORY`：返回 `memory_main_instance`、`memory_component_refs[]`，并携带 `visibility/owner_kind/owner_id/health_snapshot`。
...
- 角色固定：`super_admin`、`workspace_admin`、`user`（`R-OPS-003`）。
- `user` 禁止独立 CRUD 共享数据服务组件、禁止网关 CRUD、禁止工作空间绑定治理动作（`R-OPS-004`）。
...
### 5.1 领域模型总览（6 类一级资源 + 支撑资源）

模型分层：

- 一级资源（L1）：`DataServiceInstance`、`LowCodePlatformInstance`、`DevBoxInstance`、`GatewayInstance`、`HighCodeApplication`、`MemoryInstance`。
...
erDiagram
  ...
  RESOURCE_TEMPLATE ||--o{ MEMORY_INSTANCE : instantiate
  ...
  WORKSPACE ||--o{ MEMORY_INSTANCE : owns
  ...
  MEMORY_INSTANCE ||--o{ MEMORY_COMPONENT_REF : contains
  DATASERVICE_INSTANCE ||--o{ MEMORY_COMPONENT_REF : belongs
...
| `memory_instances` | `instance_name`、`template_id`、`version`、`entry_url`、`dependency_topology`、`main_component_ref` | `workspace_id+cluster_id+instance_name` 唯一；模板来源仅内置 `template_kind=memory` |
...
| `memory_component_refs` | `memory_instance_id,data_service_instance_id,visibility,owner_kind,owner_id,health_snapshot,last_task_id` | `memory_instance_id+data_service_instance_id` 唯一；`visibility=embedded` 时 `owner_kind/owner_id` 必填 |
...
### 5.3 关系模型（Application-Agent-DataService-Memory）
...
- `MemoryInstance` 与 `DataServiceInstance`：`1:N`，通过 `memory_component_refs` 建模；
  关系字段必须携带 `visibility`、`owner_kind/owner_id` 与 `health_snapshot`（`R-MEM-004`）。
...
- 删除记忆体时复用相同冲突判定：仅允许回收 `embedded` 或独占组件；共享组件若存在外部引用则阻断并返回 `409 REFERENCE_CONFLICT`。

聚合状态规则（对应 `R-DATA-005`）：

- Query 读路径不做跨资源实时聚合计算。
- 应用状态以应用自身最近成功/失败任务结果为准，成员状态以独立字段展示。
- 应用详情读模型固定输出：`agent_instances`、`dataservice_instances`、`entrypoints`、`release_charts`；
  记忆体读模型固定输出：`memory_main_instance`、`memory_component_refs`、`dependency_topology`、`last_orchestration_task`。
...
| `memory_instances` | `uq_mem_workspace_cluster_name(workspace_id,cluster_id,instance_name)` |
...
| `memory_component_refs` | `uq_ref_mem_ds(memory_instance_id,data_service_instance_id)` |
...
- 关系查询索引：`idx_ref_agent_app(agent_instance_id,application_id)`、
  ...
  `idx_ref_mem_ds(memory_instance_id,data_service_instance_id)`、
  `idx_ref_mem_visibility(memory_instance_id,visibility,deleted_at)`。
- 审计追踪索引：`idx_<table>_last_task(last_task_id)`、`idx_<table>_delete_task(delete_task_id)`。

模板约束：

- `resource_templates.template_kind` 新增固定值 `memory`，仅允许平台内置模板写入；
  `memory_instances.template_id` 必须引用 `template_kind=memory` 记录。
...
- 向前兼容优先：新增列使用可空 + 默认值；写路径双写，读路径分阶段切换。
...
3. `Step-3` 分阶段切换：读路径先兼容“新列优先 + 旧字段兜底”，阶段完成后切换为仅读新列。
...
- `MemoryInstance` 在组件编排部分成功时可进入 `Degraded`，需通过补偿任务恢复到 `Running`。
...
- `v1.22.0` 起 `/api/v1/instances/*` 已退出默认外部契约；
  仅历史回放包允许保留 `deprecated + x-sunset-version: v1.22.0` 元数据。
...
| 422 | `CROSS_SCOPE_REFERENCE` | 调用方具备作用域权限，但引用关系命中跨 workspace/cluster/namespace 边界 | 否 | 无 |
...
- `403` 与 `422` 分工固定为：
  - `403 FORBIDDEN_WORKSPACE/FORBIDDEN_ACTION`：权限准入失败（无作用域权限或动作越权）。
  - `422 CROSS_SCOPE_REFERENCE`：权限已通过，但引用关系跨越 `workspace/cluster/namespace`。
- 任何跨边界拒绝场景必须在 OpenAPI operation 同时声明 `403` 与 `422` 的触发条件，不得将两者混写为同一错误语义。
...
T31 `DATA_SERVICE` 接口边界（现行）：

- shared 数据服务实例的创建/查询/更新/删除与动作调用统一走
  `/api/v1/applications`（`resource_type=DATA_SERVICE`）。
- `GET /api/v1/applications?...&resource_type=DATA_SERVICE` 与
  `GET /api/v1/applications/{application_id}` 仅返回 shared 资源；
  embedded 资源不进入该列表与详情视图。
- embedded 数据服务实例的独立运维仅保留
  `{scope}/embeddeddataservices/{embedded_dataservice_id}`（`super_admin`）。
- `/dataservices*` 不属于现行外部契约；默认 OpenAPI 与验收主链路不得作为入口引用。

`resource_type=MEMORY` 契约补充（T27）：

- 创建请求体必须包含 `resource_type=MEMORY` 与内置模板引用；
  若模板来源非内置，返回 `422 SCHEMA_VALIDATION_FAILED`。
- 详情与组件查询必须返回“主实例 + 内部组件”结构，不允许退化为通用扁平组件数组。
- 动作支持遵循能力矩阵；跨 workspace/cluster/namespace 的记忆体动作统一拒绝并审计。
...
- `GET {scope}/templates?kind={dataservice|lowcode|devbox|gateway}`
...
- 创建：`POST {scope}/lowcodes`
- 导入：`POST {scope}/lowcodes/imports`
- 列表：`GET {scope}/lowcodes`
- 详情：`GET {scope}/lowcodes/{lowcode_id}`
- 更新：`PUT {scope}/lowcodes/{lowcode_id}`
- 删除：`DELETE {scope}/lowcodes/{lowcode_id}`

低代码 Helm 导入接口补充（T16）：

- `POST {scope}/lowcodes/imports` 仅允许 `super_admin` 调用，返回 `202 + Location`（响应体可带 `task_id` 供诊断）。
- `workspace_admin` 与 `user` 调用该端点统一返回 `403 FORBIDDEN_ACTION`；
  `workspace_admin` 开通内置低代码平台走 `POST {scope}/lowcodes`（非 imports）。
- 请求体最小字段：
...
| --- | --- | --- | --- | --- |
| `POST {scope}/lowcodes` | 可写，必填 | 可写，必填，仅引用 | 可写，选填（默认 `{}`） | 可写，必填 |
| `PUT {scope}/lowcodes/{lowcode_id}` | 可写 | 可写（仅允许切换引用） | 可写（全量替换） | 可写（升级/回滚） |
| `GET {scope}/lowcodes/{lowcode_id}` | 返回明文 URL | 返回引用（按角色脱敏） | 返回完整拓扑 | 返回当前版本 |
| `GET {scope}/lowcodes` | 默认返回 | 默认返回脱敏摘要 | 默认不返回；`expand=dependency_topology` 返回摘要 | 默认返回 |
...
- 关系查询：`GET {scope}/applications/{application_id}/relations`
- 发布：`POST {scope}/applications/{application_id}/releases`
- 发布包列表：`GET {scope}/applications/{application_id}/charts`
- 下载发布包：`GET {scope}/charts/{chart_id}/package`

历史兼容与退场说明（T26/T29）：

- `/api/v1/instances/*` 在 `v1.20.0~v1.21.x` 作为短期兼容路径保留，且必须标注 `deprecated: true`。
- 自 `v1.22.0` 起，`/api/v1/instances/*` 已从现行外部契约移除，不得出现在默认示例、SDK 生成入口与验收主链路中；
  历史回放场景若需引用，必须显式标注“历史基线追溯”。
...
- 发起配置变更：`POST {scope}/gateways/{gateway_id}:apply-config`
- 发起配置回滚：`POST {scope}/gateways/{gateway_id}:rollback-config`
- 查询配置历史：`GET {scope}/gateways/{gateway_id}/config-revisions`

网关配置接口最小错误码集合：

- `400 INVALID_ARGUMENT`：缺少 `config_revision/change_reason/change_ticket_id`。
- `403 FORBIDDEN_ACTION`：非 `super_admin/workspace_admin(managed_workspace)` 调用。
- `409 RESOURCE_VERSION_CONFLICT`：`resource_version` 冲突。
- `409 RESOURCE_BUSY`：同网关已有运行中配置变更任务。
- `422 SCHEMA_VALIDATION_FAILED`：配置内容或引用校验失败。
- `503 DEPENDENCY_UNAVAILABLE`：网关控制面不可达（可重试）。

T21 关键动作鉴权矩阵（API 入口口径）：
...
| 低代码平台导入（`{scope}/lowcodes/imports`） | 允许 | 拒绝 | 拒绝 | `403 FORBIDDEN_ACTION` |
...
- 角色越权固定返回 `403 FORBIDDEN_ACTION`。
- 业务前置条件不满足（如网关单例冲突、非法状态）返回 `422`。
- 并发或引用冲突返回 `409`（例如 `RESOURCE_BUSY`、`REFERENCE_CONFLICT`）。
...
- `GET {scope}/embeddeddataservices/{embedded_dataservice_id}`
- `PUT {scope}/embeddeddataservices/{embedded_dataservice_id}`
- `DELETE {scope}/embeddeddataservices/{embedded_dataservice_id}`
...
- `GET /applications/{application_id}?workspace_id=...` 在 `resource_type=MEMORY` 时必须返回：
  `memory_main_instance`、`memory_component_refs`、`dependency_topology`、
  `version`、`entry_url`、`last_orchestration_task`。
- `GET /applications/{application_id}/components?workspace_id=...` 在
  `resource_type=MEMORY` 时固定返回：
  `memory_main_instance`（单对象）+ `memory_component_refs[]`（数组）；
  组件字段至少包含 `visibility/owner_kind/owner_id/health_snapshot`。
- `memory_component_refs[]` 的 `visibility=shared` 与 `visibility=embedded`
  必须可判定，前端不得通过名称规则推断。
...
- 分支返回最小字段：
  ...
  - `MEMORY`：`component_id/component_name/component_role/status/visibility/owner_kind/owner_id/health_snapshot`。
...
- `{scope}` 固定展开为 `/api/v1/workspaces/{workspace_id}/clusters/{cluster_id}`。
...
| `instances（历史兼容包，默认不启用）` | `/api/v1/instances` | `paths/instances_compat.yaml` |
...
- 涉及跨边界引用的接口必须声明 `422 CROSS_SCOPE_REFERENCE`，并在描述中明确“权限通过但引用越界”的触发语义。
- shared 引用冲突统一声明 `409 REFERENCE_CONFLICT`；`SHARED_RESOURCE_IN_USE` 仅保留为兼容别名。
- 外部契约禁止将 `/tasks/*` 定义为进度查询主路径；进度查询统一在资源详情响应模型表达。
- `DATA_SERVICE` 外部契约仅允许 `/api/v1/applications` + `resource_type=DATA_SERVICE`；
  默认 OpenAPI 禁止新增 `/dataservices*` 路径。
...
- `MEMORY` 返回结构必须固定包含 `memory_main_instance` 与 `memory_component_refs`；
  `memory_component_refs.items` 需声明 `visibility(enum: shared|embedded)`、
  `owner_kind`、`owner_id`、`health_snapshot`。
- 动作能力矩阵必须在 operation 扩展字段显式声明 `x-action-support`，
  不支持动作统一返回 `409 ACTION_NOT_SUPPORTED` + 标准错误对象。
- `v1.22.0` 起，`/instances/*` 不得进入默认 `openapi/aether-deploy.yaml` 路径集合；
  仅历史兼容包可保留 `deprecated: true + x-sunset-version: v1.22.0`，并标记 `x-legacy-baseline: T24`。
...
- 内部请求还需携带 `actor_role/actor_id/workspace_id`；
  `workspace_admin` 仅允许命中其 `managed_workspace`，否则拒绝并审计。
...
- `gateway.config.change.requested.v1`
- `gateway.config.change.completed.v1`
- `gateway.config.rollback.completed.v1`
...
- `3.4` 控制面中的 `Redis` 组件（`aether-redis`）统一承载 `lane task stream`、`lock keyspace` 与 `eventbus stream`。
- `PostgreSQL` 与 `Redis` 必须同属于控制面部署清单；执行编排不得绕过 `Redis` 直连其他消息中间件。
...
### 8.10 记忆体编排与级联删除补偿（T27）

记忆体编排约束（`R-MEM-001~008`）：

- 创建仅允许内置模板：`template_kind=memory`，若传入非内置来源直接返回 `422 SCHEMA_VALIDATION_FAILED`。
- 组件关系持久化必须写入 `memory_component_refs`，字段最小集：
  `visibility`、`owner_kind`、`owner_id`、`health_snapshot`、`last_task_id`。
- 组件编排与宿主删除共用串行键：
  `ws:{ws}:cl:{cl}:rk:memory_instance:id:{memory_id}`，避免并发删除与补偿覆盖。
...
sequenceDiagram
  ...
  U->>API: DELETE /api/v1/applications/{application_id}?workspace_id=...&cascade=true
  API->>DB: 读取 memory_component_refs + shared 引用计数
  alt shared 组件被其他宿主引用
    API-->>U: 409 REFERENCE_CONFLICT
  else 可删除
    API->>DB: 创建 delete task(memory)
    API-->>U: 202 + Location
    W->>K: 删除主实例与 embedded 组件
    alt embedded 回收失败
      W->>DB: task=Failed + enqueue memory_component_reconcile
    else 回收成功
      W->>DB: task=Succeeded + 软删 memory_instance
    end
  end
```
...
- Service 名：
  ...
  - `P2`：shared 组件独立创建且显式传入 `service_name`，使用调用方提交名称（校验 DNS-1123 + 唯一性）。
...
type RequestContext struct {
    RequestID    string
    ActorID      string
    Role         string // super_admin | workspace_admin | user
...
- `workspace_admin`：仅允许在 `managed_workspace` 范围内执行治理与资源运维动作；越界请求拒绝并审计。
- `user`：必须满足“已关联目标工作空间 + 资源归属一致”；仅允许高代码应用/记忆体动作与低代码平台内应用入口动作；否则拒绝。
...
- 本节中的 `user` 均指三角色模型中的 `user` 角色；泛指主体统一使用“调用方”。
...
| 动作 | `super_admin` | `workspace_admin(managed_workspace)` | `user(associated workspace)` |
| --- | --- | --- | --- |
| 工作空间/集群/仓库绑定变更与回滚 | 允许 | 允许 | 拒绝 |
| 工作空间成员管理（平台管理面动作） | 允许 | 允许 | 拒绝 |
| 共享数据服务组件 CRUD | 允许 | 允许 | 拒绝 |
| 低代码平台开通/关闭（内置） | 允许 | 允许 | 拒绝 |
| 网关实例 CRUD | 允许 | 允许 | 拒绝 |
| 高代码应用 CRUD | 允许 | 允许 | 允许 |
| 记忆体 CRUD | 允许 | 允许 | 允许 |

T27 `user` 角色能力边界单口径定稿（`R-OPS-004`）：

- `user` 仅允许操作 `HIGH_CODE_APP` 与 `MEMORY` 两类一级资源，并访问低代码平台内应用入口。
- `user` 永久禁止独立 CRUD `DATA_SERVICE`、`GATEWAY`、低代码平台实例本体与任何 internal 绑定治理接口。
- `MEMORY` 关联的 `embedded` 组件仅允许通过宿主记忆体路径触发运维/回收，不授予独立组件运维权限。
...
| --- | --- | --- | --- | --- |
| 通过 `applications` 单接口读取/更新/删除 `embedded`（`resource_type=DATA_SERVICE`） | 拒绝（`404`） | 拒绝（`404`） | 拒绝（`404`） | `applications` 下 DATA_SERVICE 仅暴露 shared；embedded 不可见 |
| 通过 `embeddeddataservices` 独立接口读取/更新/删除 `embedded` | 允许 | 拒绝（`403 FORBIDDEN_ACTION`） | 拒绝（`403 FORBIDDEN_ACTION`） | 超管专用路径，满足需求权限矩阵“嵌入式组件独立运维” |
| 通过高代码应用宿主执行 `embedded` 运维（升级/重启/回收） | 允许 | 允许（需有该应用权限） | 允许（需有该应用权限） | 以宿主动作鉴权，不单独下发 `embedded` 权限 |
| 通过低代码平台宿主执行 `embedded` 运维 | 允许 | 允许（限 managed workspace） | 拒绝（`403 FORBIDDEN_ACTION`） | 低代码平台治理仅 `super_admin/workspace_admin` |
| 通过记忆体宿主执行 `embedded` 运维（升级/重启/回收） | 允许 | 允许（需有该记忆体权限） | 允许（需有该记忆体权限） | 以宿主记忆体动作鉴权；禁止脱离宿主复用绑定 |
| 宿主删除触发 `embedded` 回收补偿 | 允许 | 允许触发宿主删除；补偿由系统执行 | 允许触发宿主删除；补偿由系统执行 | 审计记录操作者与系统补偿任务 |
...
| 操作 `super_admin/workspace_admin` 限定动作 | `403` | `FORBIDDEN_ACTION` |
...
- 控制面与集群、仓库通信统一 TLS。
- 仅控制面服务账号具备 Secret 明文读取权限；用户只可引用不可读取（`S-SEC-002`）。
...
| `task_id` | 关联任务 ID（可空） |
| `actor_id` | 操作者 ID |
| `actor_role` | 操作者角色（`super_admin/workspace_admin/user/system`） |
...
- 所有跨边界拒绝都必须记录审计事件（`result=failed`）。
- 权限准入失败（无工作空间权限、动作越权）返回
  `403 FORBIDDEN_WORKSPACE/FORBIDDEN_ACTION`。
- 权限已通过但引用跨 `workspace/cluster/namespace` 返回
  `422 CROSS_SCOPE_REFERENCE`。

`403/422` 边界矩阵（T32）：

| 场景 | 返回码 | 业务码 | 审计要求 |
| --- | --- | --- | --- |
| 调用方不在 `managed_workspace` 或作用域越权 | `403` | `FORBIDDEN_WORKSPACE` | 必须记录拒绝原因与目标作用域 |
| 调用方角色无对应动作权限 | `403` | `FORBIDDEN_ACTION` | 必须记录角色、动作、资源类型 |
| 具备权限但请求引用了其他 workspace/cluster/namespace 资源 | `422` | `CROSS_SCOPE_REFERENCE` | 必须记录 `requested_scope/resolved_scope` 差异 |
...
| 维度 | shared | embedded |
| --- | --- | --- |
| 创建入口 | 统一 `applications` 接口（`resource_type=DATA_SERVICE`） | 仅随宿主资源创建 |
| 列表可见性 | 出现在共享列表/选择器 | 不出现在共享列表/选择器 |
| 运维入口 | `super_admin/workspace_admin` 可独立更新/删除；`user` 禁止 | 仅通过宿主运维 |
| 复用能力 | 可被多个应用引用 | 仅宿主可用 |
| 回收策略 | 非级联默认保留 | 宿主删除时自动回收 |

术语约束（T35）：

- 本节中的 `user` 均指三角色模型中的 `user` 角色，不表示“任意平台用户”。

接口层过滤规则：

- `GET /api/v1/applications?workspace_id={workspace_id}&resource_type=DATA_SERVICE`
  仅返回 `visibility=shared` 的数据服务实例。
- `GET/PUT/DELETE /api/v1/applications/{application_id}?workspace_id={workspace_id}` 在目标资源
  为 `embedded` 数据服务实例时统一返回 `404 RESOURCE_NOT_FOUND`。
- 仅
  ...
  `GET /api/v1/applications/{application_id}/components?workspace_id={workspace_id}`（当 `resource_type=MEMORY`）
  的详情读模型返回嵌入式关系。
- 超管独立运维仅走
  `GET/PUT/DELETE /workspaces/{workspace_id}/clusters/{cluster_id}/embeddeddataservices/{embedded_dataservice_id}`；
  `workspace_admin` 与 `user` 访问该路径均返回 `403 FORBIDDEN_ACTION`。
- 同 workspace 不按创建者隔离：高代码应用与记忆体操作不校验 `owner_user_id`，
  仅校验 `workspace` 归属 + 角色动作许可 + 资源状态前置条件。
...
- 权限判定：
  ...
  - 低代码宿主：`lowcode.manage`（`super_admin/workspace_admin`）可触发 `embedded` 运维。
  - 记忆体宿主：`memory.update/delete` 通过后允许执行对应 `embedded` 运维。
- 审计动作：
...
- T21 共享操作审计最小字段补充：
  `actor_role`、`actor_id`、`workspace_id`、`resource_kind`、`resource_id`、`action`、`result`、`request_id`。
...
stateDiagram-v2
  [*] --> Active
  Active --> Frozen: unbind request (super_admin/workspace_admin + confirm)
...
- 全流程仅 `super_admin` 或 `workspace_admin(managed_workspace)` 可触发，且需要二次确认。
...
- `aether_memory_instance_total`（`Gauge`）：
  `labels=workspace_bucket/cluster_bucket/status`；容量门槛 `>=5000` 实例。
- `aether_memory_component_relation_total`（`Gauge`）：
  `labels=workspace_bucket/visibility`；容量门槛 `>=20000` 关系，支持 `shared/embedded` 分维度观测。
...
- 记忆体组件查询延迟（`R-MEM-007` 联动）：
  在 `>=20000` 组件关系规模下，`GET /applications/{application_id}/components`
  的 `resource_type=MEMORY` 分支目标 `P95<=1s`、`P99<=2.5s`。
...
- 资源实例下限：
  ...
  gateway `>=5000`；application `>=5000`；memory `>=5000`；agent `>=10000`。
- 记忆体关系规模下限：`memory_component_refs >=20000`（`shared+embedded` 总量）。
- 任务吞吐：日均异步任务 `>=10000`。
...
| NFR-014 | 跨边界请求拒绝并审计 | 三阶段跨边界校验 + `403/422` 语义分工（含 `CROSS_SCOPE_REFERENCE`）+ 强制审计 |
...
`{scope}` 在本章统一展开为 `/api/v1/workspaces/{workspace_id}/clusters/{cluster_id}`。
...
| R-ENV-006~007 | 绑定变更/回滚+凭证闭环（`TC-ENV-04/05、DBX-03`） | `super_admin` 或授权 `workspace_admin` 可用；回滚成功；复用 `task_id`；镜像 push 可追溯 | 接口响应、任务结果、binding 快照、审计 |
| R-ENV-009~010 | 解绑冻结与恢复窗口（`TC-ENV-03`） | 状态流转正确；仅 `super_admin/workspace_admin(managed_workspace)` 执行且二次确认 | 状态机日志、审计日志、操作录屏 |
| R-ABS-001~004 | 新资源类型接入不改主流程（`TC-ABS-01`） | 仅新增 schema+adapter 即可跑通 CUD；任务、鉴权、审计代码路径无分叉 | PR diff、单测、集成测试报告 |
| R-ABS-005~008 | 标签与 L1 统计口径（`TC-ABS-02`） | K8s 对象带完整标签；控制面不以 Pod 为主资源；报表按 6 类 L1 聚合 | K8s 清单、API 契约测试、配额报表 |
| R-DSP-001~007 | 共享组件创建、扩缩容、升级回滚（`TC-DSP-01`） | 组件 CRUD 与运维动作均走异步任务；Service 命名规则符合策略 | 任务记录、状态快照、Service 清单 |
| R-DSP-008~013 | embedded 隔离与宿主回收（`TC-DSP-02/04`） | `embedded` 不进共享池；`applications?resource_type=DATA_SERVICE` 仅返回 shared；宿主删除自动回收 | 列表响应、关系表、权限审计 |
| R-LCP-001~004 | 低代码平台内置/导入 Helm（`TC-LCP-01/05`） | 仅 Helm 导入；内置开通/关闭允许 `super_admin/workspace_admin`，导入仅 `super_admin`；实例 CRUD（含 `DELETE`）与升级回滚成功 | API 响应、任务日志、权限报告 |
| R-LCP-005~006、008~010 | 低代码依赖归属与多实例（`TC-LCP-02`） | 依赖组件标记 `embedded`；多实例默认关闭；用户仅查看平台实例与入口并走平台内应用入口 | 拓扑详情、列表响应、RBAC、策略审计 |
...
| R-GTW-001~006 | 网关单例、配置变更与回滚审计、发布前校验（`TC-GTW-01`） | workspace+cluster 仅 1 网关实例；配置生效失败自动回滚；发布前网关可用校验 | 唯一约束验证、任务日志、审计日志、发布校验日志 |
...
| R-MEM-001~008 | 记忆体创建、关系编排与级联冲突（`TC-MEM-01`、`TC-MEM-02`） | 记忆体仅内置模板创建；组件关系返回“主实例+内部组件”并标注 `shared\|embedded`；`cascade=true` 冲突统一 `409 REFERENCE_CONFLICT` | API 响应样本、关系快照、任务结果、冲突审计 |
| R-DATA-001~009 | 关系查询与变更审计（`TC-DATA-01/02/03`） | 详情与 `relations` 返回 4 类关系；`MEMORY` 返回主实例+内部组件；绑定/解绑/级联删除可审计 | 详情响应、relations 响应、组件查询样本、审计检索 |
...
| --- | --- |
| 模板查询 | `GET {scope}/templates?kind=dataservice` |
| DevBox 发布记录创建 | `POST {scope}/devboxes/{devbox_id}/publishes` |
| 发布记录详情 | `GET {scope}/publishes/{publish_id}` |
| 高代码应用关系查询 | `GET {scope}/applications/{application_id}/relations` |
| 高代码应用发布包列表 | `GET {scope}/applications/{application_id}/charts` |
| 发布包下载 | `GET {scope}/charts/{chart_id}/package` |
...
| 配额统计/审计报表口径 | `TC-ABS-03` | 配额与报表按 6 类一级资源聚合，支撑资源仅作为明细展示 | 报表接口响应、SQL 核对结果 |
| 外部导入闭环 | `TC-PKG-02` | 下载校验通过、外部 `helm install` 成功、运行态验证通过 | 下载摘要、命令输出、`helm status`、探活结果 |
| embedded 运维权限统一 | `TC-DSP-03` | `applications` 单接口对 embedded 返回 `404`；宿主路径按宿主权限执行 | API 调用记录、RBAC 测试、审计事件 |
...
| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| embedded 权限对齐 | `TC-DSP-04` | `applications/{application_id}(DATA_SERVICE)=404`；admin 接口 `200`，user `403 FORBIDDEN_ACTION` | API 记录、权限测试、审计事件 |
| relations 端点补齐 | `TC-DATA-02` | `GET {scope}/applications/{application_id}/relations` 返回关系与 owner 字段；可生成契约测试 | 接口响应、OpenAPI 路径、契约测试 |
...
| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| 网关配置变更与回滚审计定稿 | `TC-GTW-01-A` | 配置变更任务可执行；失败自动回滚至稳定 `config_revision`；审计链路完整 | 任务日志、回滚结果、审计 |
...
| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| 四字段模型与契约一致性 | `TC-LCP-04-A` | `GET/POST/PUT {scope}/lowcodes` 四字段与 `5.2/5.4` 一致，无字段漂移 | OpenAPI 契约测试、DDL 校验、接口响应样本 |
| 历史数据迁移与分阶段切换 | `TC-LCP-04-B` | 按“加字段 -> 回填 -> 读切换 -> 写强校验”完成；失败可回滚到兼容读模式 | 迁移日志、回填结果、回滚演练记录 |
...
- `T16-A`：
  `POST {scope}/lowcodes/imports` 仅 `super_admin` 可调用；
...
| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| 三角色授权关系与生效时机 | `TC-SEC-03-A` | `super_admin/workspace_admin/user` 角色边界一致；`workspace_admin` 仅能命中 `managed_workspace`；权限移除后 API 与 Worker 执行前均拒绝 | 鉴权日志、任务拒绝样本、审计事件 |
| 治理动作权限收敛 | `TC-ENV-07` | 集群/仓库绑定与解绑、回滚支持 `super_admin` 与授权 `workspace_admin`；`user` 调用返回 `403 FORBIDDEN_ACTION` | Internal API 响应、错误样本、审计日志 |
| `user` 角色能力边界收敛 | `TC-OPS-10` | `user` 角色仅可执行高代码应用/记忆体动作与低代码平台内应用入口；独立 CRUD `applications(resource_type=DATA_SERVICE\|GATEWAY\|LOW_CODE_PLATFORM)` 与 `lowcodes(import\|create\|update\|delete)` 被拒绝 | RBAC 测试、接口响应、审计事件 |
...
#### 14.1.15 T23 网关配置口径收敛验收样例

| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| 网关配置变更与回滚审计口径收敛 | `TC-GTW-02-A` | `4.6/7.3/7.6` 仅保留“变更+回滚+审计”约束，无策略算法描述 | 文档 diff、架构评审记录 |
| 配置失败自动回滚闭环 | `TC-GTW-02-B` | 配置生效失败后自动回滚至稳定 revision，任务状态与审计一致 | 任务结果、回滚日志、审计事件 |
| 术语一致性与回标同步 | `TC-GTW-02-C` | 文本检索无旧术语残留；`14.4.20/15.3.3` 证据链闭环 | 检索记录、回标记录、开放项关单记录 |

#### 14.1.16 T24 双视图 API 与 ID 语义收敛验收样例（历史基线）

说明：本节仅用于历史追溯回放，不作为现行发布门禁；现行门禁以 `14.1.18` 为准。

| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| `instance_id/application_id` 语义冻结 | `TC-DATA-04-A` | 术语、路径、OpenAPI 参数命名不混用；历史误用示例已修正 | 文档 diff、OpenAPI lint 报告 |
| 实例视图契约收敛 | `TC-OPS-12-A` | 历史回放环境中 `POST/GET /api/v1/instances`、`GET/PUT/DELETE /api/v1/instances/{instance_id}` 与 `start/stop/restart` 可复现实例视图行为 | 契约测试、接口回放、错误样本 |
| 应用视图与应用下实例查询契约收敛 | `TC-DATA-04-B` | 历史回放环境中 `GET /api/v1/applications/{application_id}/instances` 按应用类型返回分支结构；空集返回 `200` | 查询响应样本、分支断言报告 |
...
| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| 单接口主路径收敛 | `TC-DATA-05-A` | 外部主接口仅使用 `/api/v1/applications`；`/api/v1/instances/*` 在 `v1.22.0` 已退出现行契约，仅允许历史回放标注 | OpenAPI lint、接口回放、文档检索记录 |
| 统一 ID 语义收敛 | `TC-DATA-05-B` | `application_id` 在术语、路径、参数对象中单口径；6 类 `resource_type` 均可映射实体主键 | 文档 diff、契约测试、映射校验报告 |
| 组件查询语义替换 | `TC-DATA-05-C` | `GET /api/v1/applications/{application_id}/components` 覆盖 6 类返回分支；空集返回 `200` | 接口响应样本、分支断言报告 |
| 动作能力矩阵收敛 | `TC-OPS-14-A` | `start\|stop\|restart` 按 `resource_type` 执行；不支持动作统一返回 `409 ACTION_NOT_SUPPORTED` | 动作探测测试、错误样本、审计日志 |

#### 14.1.19 T27 记忆体（MEMORY）并入与边界收敛验收样例

| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| `mem0` 迁移与 6 类口径一致性 | `TC-MEM-01-A` | 数据服务固定类型不再包含 `mem0`；`MEMORY` 纳入 6 类 L1 统一统计与接口过滤 | 文档检索记录、接口样本、报表快照 |
| 记忆体关系与组件查询闭环 | `TC-MEM-01-B` | `GET /api/v1/applications/{application_id}/components` 在 `MEMORY` 分支返回 `memory_main_instance + memory_component_refs[]`，并带 `shared\|embedded` 与 `owner` 字段 | 查询响应样本、契约断言、关系表快照 |
| 记忆体级联删除冲突语义 | `TC-MEM-02-A` | `cascade=true` 且共享子实例被外部引用时返回 `409 REFERENCE_CONFLICT`；无误删 | 删除响应样本、引用计数校验、审计日志 |
| 记忆体权限边界与观测指标 | `TC-MEM-02-B` | `user` 仅可操作 `HIGH_CODE_APP/MEMORY`；`dataservice` 独立 CRUD 拒绝；`12.2/13.1` 记忆体容量与关系指标可观测 | RBAC 测试、错误样本、监控截图 |

#### 14.1.20 T28 编号迁移与追踪治理验收样例

| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| `R-DSP/R-DATA` 新旧编号迁移表冻结 | `TC-TRACE-01-A` | `14.4.25` 提供旧编号失效、新编号生效的单一映射；同一语义仅保留一个有效编号 | 迁移表、评审记录、文档检索记录 |
| 验收矩阵与覆盖回标编号同步 | `TC-TRACE-01-B` | `14.1` 与 `14.4.3/14.4.25` 对 `R-DSP-010~013`、`R-DATA-003~011` 引用一致，无跨章节错链 | 验收矩阵 diff、覆盖回标 diff、交叉核对清单 |
| 失效编号回归扫描闭环 | `TC-TRACE-01-C` | 全文扫描无失效编号作为“有效引用”；历史语义仅在迁移表保留 | 扫描结果、修复清单、复核记录 |
| 变更流程阻断门禁生效 | `TC-TRACE-01-D` | 涉及编号重排的变更未同步正文/验收/回标时，评审门禁阻断并禁止进入实现拆分 | 变更单样本、门禁日志、评审结论 |

#### 14.1.21 T29 全文一致性冲突与歧义收敛验收样例
...
#### 14.1.23 T31 `DATA_SERVICE` 接口口径统一验收样例

| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| `DATA_SERVICE` 单接口 canonical path 收敛 | `TC-DSP-05-A` | shared 数据服务的 CRUD/查询/动作仅通过 `/api/v1/applications`（`resource_type=DATA_SERVICE`）执行 | 契约测试、接口响应样本、OpenAPI diff |
| shared/embedded 接口可见性与权限收敛 | `TC-DSP-05-B` | `applications` 单接口对 embedded 返回 `404`；`embeddeddataservices` 仅 `super_admin` 可用 | RBAC 测试、错误响应样本、审计事件 |
| 验收与回标证据单口径 | `TC-TRACE-04-A` | `14.1/14.4` 不再出现 `applications` 与 `dataservices` 并行主口径 | 覆盖回标 diff、检索记录、评审结论 |

#### 14.1.24 T32 跨边界错误码词典补齐验收样例
...
#### 14.1.26 T34 执行底座部署视图一致性修复验收样例

| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| 部署视图补齐 Redis 控制面组件 | `TC-OPS-17-A` | `3.4` 控制面明确包含 `PostgreSQL + Redis`；`API/Worker/Scheduler` 与 Redis 连接关系可追踪 | 架构图 diff、部署清单、评审记录 |
| 执行编排与部署视图单口径 | `TC-OPS-17-B` | `3.2/3.4/8.1.1` 均明确 Redis 承载队列/锁/EventBus，且不存在独立消息中间件旁路描述 | 文档检索记录、章节交叉核对清单 |
| 验收与回标证据联动 | `TC-TRACE-06-A` | `14.1.26` 与 `14.4.31` 引用章节一致，满足可验收闭环 | 验收矩阵 diff、覆盖回标 diff、评审结论 |

#### 14.1.27 T35 “用户”术语去歧义与角色语义统一验收样例

| 缺口 | 用例 ID | 通过条件 | 证据 |
| --- | --- | --- | --- |
| `user` 角色词义冻结 | `TC-TRACE-07-A` | `2.5/10.2/11.3` 中 `user` 均唯一映射到三角色模型中的 `user` 角色，不再承载“泛指平台用户”语义 | 章节检索记录、评审结论 |
| 命名文案去歧义 | `TC-DSP-06-A` | `4.3/9.4` 的 Service 命名策略使用“调用方提交名称”，且 DNS-1123 与同 namespace 唯一性约束保持不变 | 文档 diff、规则核对记录 |
| 验收与回标语义一致性 | `TC-TRACE-07-B` | `14.1/14.4` 角色边界条目中，`user` 均显式指角色；泛指主体统一为“调用方/工作空间成员” | 验收矩阵 diff、覆盖回标 diff、评审记录 |
...
| KS-12 | embedded 独立接口与宿主派生运维 | R-DSP-011、R-DSP-012 | `applications(resource_type=DATA_SERVICE)` 对 embedded 返回 `404`；`embeddeddataservices` 仅 `super_admin`；宿主路径保留审计 |
| KS-13 | 记忆体 `cascade=true` 删除冲突阻断 | R-MEM-006、R-DSP-013 | 共享子实例存在外部引用时返回 `409 REFERENCE_CONFLICT`；补偿任务可重放 |
...
2. 控制面分批发布：先发布 `aether-api/aether-worker` 小批次副本（建议 10%）。
3. 工作空间分批发布：按 workspace 批次放量（建议 5% -> 20% -> 50% -> 100%）。
...
| `R-MEM` | 4.9、5.2、5.3、5.4、6.1、7.3~7.5、8.10、10.2、11.3、14.1 |
...
| R-ENV-004 | 4.1、5.7、6.5、7.6、10.2、11.1、11.2、14.1.24、14.1.27、14.4.29、14.4.32 | 已覆盖 | 已在 4.1、7.3、10.2、11.2、14.1.27 定稿 `user` 角色边界与跨边界拒绝口径：权限越权返回 `403`，引用越界返回 `422 CROSS_SCOPE_REFERENCE`，并统一审计。 |
...
| R-ENV-010 | 4.1、5.7、6.5、7.6、11.1、11.2 | 已覆盖 | 已在 4.1、7.6、11.5 定稿解绑权限：`super_admin` 或授权 `workspace_admin`，且必须二次确认并审计。 |
...
| R-ABS-007 | 3.3、4.2、5.1、8.1~8.6 | 已覆盖 | 已在 2.6、4.2 补齐“6 类一级资源用于配额统计与审计报表”的聚合口径与查询路径。 |
...
| R-DSP-011 | 4.3、5.2、5.3、9.1~9.5、10.2、11.3、14.1.23、14.1.27、14.4.28、14.4.32 | 已覆盖 | 已在 10.2、11.3、14.1.23、14.1.27 固化两条运维路径：超管独立运维 + 宿主派生权限，并完成角色术语去歧义。 |
| R-DSP-012 | 4.3、5.2、5.3、9.1~9.5、10.2、11.3、14.1.23、14.1.27、14.4.28、14.4.32 | 已覆盖 | 已在 10.2、11.3、14.1.23、14.1.27 固化隔离：`applications` 单接口仅返回 shared，embedded 统一 `404`，并完成角色术语去歧义。 |
...
| R-LCP-002 | 4.4、5.2、7.3、9.1~9.5、10.2、11.3 | 已覆盖 | 已在 4.4、7.3、10.2 定稿开通/关闭权限：`super_admin` 与授权 `workspace_admin` 允许，`user` 拒绝。 |
...
| R-GTW-006 | 4.6、5.2、9.1~9.5、11.2 | 已覆盖 | 已在 4.6 定稿“配置变更 + 回滚 + 审计”约束，并在 14.1.15 补齐验收闭环。 |
...
| R-MEM-001 | 4.9、5.2、6.1、7.3、10.2、14.1.19 | 已覆盖 | 已定稿 `MEMORY` 作为一级资源并纳入统一动作与权限矩阵。 |
| R-MEM-002 | 4.9、5.2、5.3、7.4、8.10、14.1.19 | 已覆盖 | 已定义“主实例 + 内部组件实例”关系模型与组件查询返回结构。 |
| R-MEM-003 | 4.9、5.2、7.3、8.10 | 已覆盖 | 已限定记忆体仅允许内置模板来源，不支持外部导入与用户上传模板。 |
| R-MEM-004 | 4.9、5.2、5.3、7.4、11.3 | 已覆盖 | 已在关系模型与查询契约中固定 `shared\|embedded`、`owner_kind/owner_id` 与健康快照字段。 |
| R-MEM-005 | 4.9、5.3、7.4、11.2、11.3 | 已覆盖 | 已固化同 `workspace+cluster+namespace` 关联限制与越界拒绝语义。 |
| R-MEM-006 | 4.9、5.3、8.10、11.4、14.1.19 | 已覆盖 | 已定稿 `cascade=true` 冲突阻断与补偿任务链路。 |
| R-MEM-007 | 4.9、7.4、7.5、14.1.19 | 已覆盖 | 已定义记忆体组件查询返回“主实例 + 内部组件列表”并携带引用关系。 |
| R-MEM-008 | 4.9、7.4、12.2、13.1、14.1.19 | 已覆盖 | 已补齐记忆体运维视图字段与可观测/性能预算约束。 |
...
| R-DATA-003 | 4.9、5.2、5.3、7.4、11.3、14.1.19 | 已覆盖 | 已在 4.9、5.3、7.4 定义记忆体“主实例 + 内部组件实例”关系，并标注 `shared\|embedded` 与 `owner_kind/owner_id`。 |
| R-DATA-004 | 4.9、5.3、7.3、7.4、11.3、12.6 | 已覆盖 | 已在 4.9、5.3 固化关系查询返回 `visibility` 与 `owner_kind/owner_id`。 |
| R-DATA-005 | 4.9、5.3、7.4、11.3、12.6 | 已覆盖 | 已在 4.9、7.4 固化 Query 非聚合规则：状态由应用自身任务结果驱动。 |
| R-DATA-006 | 4.9、5.5、7.3、11.3、12.6 | 已覆盖 | 已在 5.5 固化软删审计字段：`deleted_at/deleted_by/delete_task_id`。 |
...
| R-OPS-004 | 4.10、7.1~7.6、8.1~8.6、10.2、11.2、11.3、12.6、14.1.24、14.1.27、14.4.29、14.4.32 | 已覆盖 | 已在 7.2、10.2、11.2、11.3、14.1.27 定稿跨边界与越权拒绝分工，并冻结 `user` 为角色语义：越权 `403 FORBIDDEN_ACTION`，引用越界 `422 CROSS_SCOPE_REFERENCE`。 |
| R-OPS-005 | 3.2、3.4、4.10、7.1~7.6、8.1~8.6、12.6、14.1.26、14.4.31 | 已覆盖 | 已在 8.1 固化串行化键模型与同资源串行执行，并在 3.2/3.4/8.1.1 对齐 Redis 队列与锁的部署/编排表达。 |
...
| R-OPS-009 | 3.2、3.4、4.10、7.1~7.6、8.1~8.6、12.6、14.1.26、14.4.31 | 已覆盖 | 已在 8.6 固化补偿触发与一致性策略，并在 3.2/3.4/8.1.1 明确 EventBus 运行态由 Redis 承载。 |
...
| R-OPS-012 | 3.5、7.1、7.3、7.5、14.1.18、14.1.19、14.1.21、14.1.22、14.1.23、14.4.23、14.4.24、14.4.26、14.4.27、14.4.28 | 已覆盖 | T26/T29/T30/T31 已补齐统一应用接口 OpenAPI 清单（CRUD + actions + components），并收敛主路径时序示例与 DATA_SERVICE 接口边界。 |
...
| NFR-003 | 3.2、3.4、10、11、12、13、14.1、14.2、14.4.31 | 已覆盖 | 已在 12.2、12.4、13.1 固化队列等待时延指标与告警，并在 3.4 明确对应 Redis 运行组件。 |
...
| NFR-008 | 10、11、12、13、14.1、14.2 | 已覆盖 | 已在 13.3 固化 lowcode/devbox/gateway/memory 容量目标。 |
| NFR-009 | 10、11、12、13、14.1、14.2 | 已覆盖 | 已在 13.3 固化 application/agent 与 memory 组件关系规模目标。 |
...
| NFR-014 | 10、11、12、13、14.1、14.2、14.4.29 | 已覆盖 | 已在 11.2、13.4、14.1.24 固化跨边界拒绝与强制审计，`403/422` 分工与错误码词典一致。 |
...
| `applications relations` canonical path 与 OpenAPI 落盘补齐 | R-DATA-001~009（接口契约要求；编号迁移见 14.4.25） | 待补充 | 已覆盖 | 7.3、7.5、14.1.1、14.1.5 |
...
| --- | --- | --- | --- | --- |
| 网关配置变更与回滚审计口径定稿（去策略算法化、保留可追溯边界） | R-GTW-006 | 待补充 | 已覆盖 | 4.6、14.1.15、15.3.3 |
...
| 历史数据迁移与分阶段切换策略补齐（可回滚） | R-LCP-007 | 待补充 | 已覆盖 | 5.8、14.1.7 |
...
#### 14.4.18 T21 三角色权限模型重构回标记录（2026-02-27）

| 回标项 | 关联需求 | T21 前状态 | T21 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| 三角色基线与 `managed_workspace` 授权关系落盘（API + Worker 双重生效） | R-OPS-003、R-ENV-002 | 待补充 | 已覆盖 | 2.5、4.1、10.1、10.2 |
| 治理动作权限收敛（绑定/解绑/回滚、低代码开关、网关 CRUD） | R-ENV-006、R-ENV-010、R-LCP-002、R-GTW-004 | 待补充 | 已覆盖 | 4.1、4.4、4.6、7.3、7.6 |
| `user` 角色能力边界收敛与禁用动作统一拦截 | R-ENV-004、R-OPS-004 | 待补充 | 已覆盖 | 4.7、7.3、10.2、11.3 |
...
#### 14.4.20 T23 网关配置口径收敛回标记录（2026-02-27）

| 回标项 | 关联需求 | T23 前状态 | T23 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| `R-GTW-006` 正文口径收敛为“配置变更+回滚+审计” | R-GTW-006 | 待补充 | 已覆盖 | 4.6、7.3、7.6 |
| 全文术语清理（去除历史术语并保持可执行） | R-ABS-004 | 待补充 | 已覆盖 | 1.1、5.8、14.1、14.3、15.3 |
| 验收与覆盖回标改写 | R-GTW-006、R-OPS-010 | 待补充 | 已覆盖 | 14.1.15、14.4.3、14.4.20 |
| 变更治理新增术语一致性复核门禁 | R-OPS-010 | 待补充 | 已覆盖 | 15.2.2（第 20 条）、15.3.3、`docs/decision.md`（ADR-099） |

#### 14.4.21 T24 双视图 API 与 ID 语义收敛回标记录（2026-02-27，历史基线）

说明：本节仅用于历史追溯，不作为现行发布门禁；现行门禁以 `14.1.18` 与 `14.1.21` 为准。

| 回标项 | 关联需求 | T24 前状态 | T24 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| `instance_id/application_id` 术语与路径参数冻结 | R-DATA-010、R-OPS-012 | 待补充 | 已覆盖 | 1.2、2.6、7.1、7.3 |
| 实例视图接口清单与动作矩阵落盘（历史回放） | R-OPS-012、R-OPS-013 | 待补充 | 已覆盖 | 7.1、7.3、7.5、14.1.16 |
...
| 单接口主路径与过滤契约收敛（`applications-only`） | R-DATA-010、R-OPS-012 | 待补充 | 已覆盖 | 7.1、7.3、7.5、14.1.18、14.1.21 |
...
#### 14.4.24 T27 记忆体（MEMORY）一级资源并入回标记录（2026-02-27）

| 回标项 | 关联需求 | T27 前状态 | T27 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| 术语与抽象口径从 5 类切换到 6 类，`MEMORY` 并入一级资源 | R-ABS-001、R-ABS-007 | 待补充 | 已覆盖 | 1.2、2.1、3.2、3.3、5.1 |
| `mem0` 从数据服务组件口径迁移为记忆体资源能力 | R-DSP-001、R-DSP-010~013 | 待补充 | 已覆盖 | 4.3、5.2、5.4、10.2、11.3 |
| 记忆体关系模型与编排补偿落盘（主实例+内部组件） | R-MEM-002~008、R-DATA-003/004 | 待补充 | 已覆盖 | 4.9、5.3、7.4、8.10、14.1.19 |
| 权限/NFR/验收治理闭环（角色边界 + 容量指标 + 回标） | R-OPS-004、NFR-008、NFR-009 | 待补充 | 已覆盖 | 10.2、12.2、13.1、13.3、14.1.19、15.2 |

#### 14.4.25 T28 `R-DSP/R-DATA` 编号迁移与追踪治理回标记录（2026-02-27）

`R-DSP/R-DATA` 编号迁移对照表（冻结）：

| 需求簇 | 旧编号（失效） | 新编号（有效） | 语义说明 | 同步章节 |
| --- | --- | --- | --- | --- |
| `R-DSP` | `R-DSP-010` | `R-DSP-011` | 嵌入式默认仅宿主访问 + `super_admin` 独立运维 + 禁止复用绑定 | 14.1、14.4.3、15.2 |
| `R-DSP` | `R-DSP-011` | `R-DSP-012` | shared/embedded 在 API/列表/权限隔离 | 14.1、14.4.3、15.2 |
| `R-DSP` | `R-DSP-012` | `R-DSP-013` | 删除宿主时回收嵌入式组件 | 14.1、14.4.3、15.2 |
| `R-DSP` | 无（新增） | `R-DSP-010` | 记忆体嵌入式组件标记 `embedded_by_memory` | 14.1、14.4.3、15.2 |
| `R-DATA` | `R-DATA-003` | `R-DATA-004` | 关系查询区分 `shared\|embedded` 并返回 `owner_kind/owner_id` | 14.1、14.4.3、15.2 |
| `R-DATA` | `R-DATA-004` | `R-DATA-005` | Query 不做跨资源实时聚合计算 | 14.1、14.4.3、15.2 |
| `R-DATA` | `R-DATA-005` | `R-DATA-006` | 软删保留审计字段 | 14.1、14.4.3、15.2 |
| `R-DATA` | `R-DATA-006` | `R-DATA-007` | embedded 不进入共享选择器 | 14.1、14.4.3、15.2 |
| `R-DATA` | `R-DATA-007` | `R-DATA-008` | 应用查看页字段集合 | 14.1、14.4.3、15.2 |
| `R-DATA` | `R-DATA-008` | `R-DATA-009` | 关系变更审计轨迹 | 14.1、14.4.3、15.2 |
| `R-DATA` | `R-DATA-009` | `R-DATA-010` | 统一 `applications` 视图覆盖 6 类资源 | 14.1、14.4.3、15.2 |
| `R-DATA` | `R-DATA-010` | `R-DATA-011` | `GET /api/v1/applications/{application_id}/components` 分支查询 | 14.1、14.4.3、15.2 |
| `R-DATA` | 无（新增） | `R-DATA-003` | 记忆体主实例+内部组件关系持久化 | 14.1、14.4.3、15.2 |

| 回标项 | 关联需求 | T28 前状态 | T28 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| 新旧编号迁移表落盘并冻结唯一有效编号 | R-DSP-010~013、R-DATA-003~011 | 待补充 | 已覆盖 | 14.4.25 |
| 验收矩阵编号同步（含 `R-DATA-009` 与 `R-DATA-010/011`） | R-DATA-003~011 | 待补充 | 已覆盖 | 14.1、14.1.20、14.4.3 |
| 覆盖核对表编号重排与语义对齐 | R-DSP-010~013、R-DATA-003~011 | 待补充 | 已覆盖 | 14.4.3、14.4.25 |
| 交付与变更流程新增“编号迁移一致性”阻断门禁 | R-OPS-010 | 待补充 | 已覆盖 | 15.1、15.2.2（第 25 条）、`docs/decision.md`（ADR-104） |

#### 14.4.26 T29 全文一致性冲突与歧义收敛回标记录（2026-02-27）

| 回标项 | 关联需求 | T29 前状态 | T29 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| `R-ENV-005` 语义解耦：先选 `workspace/cluster` 与路径形态分离 | R-ENV-005 | 待补充 | 已覆盖 | 4.1、7.1、7.3、14.1.21 |
| 幂等判重统一为 `idempotency_scope + request_fingerprint`（含 Secret 章节） | R-OPS-002 | 待补充 | 已覆盖 | 2.6、7.1、8.4、10.3、14.1.21 |
| `/instances` 兼容退场时态统一为“`v1.22.0` 已退出现行外部契约” | R-DATA-010、R-OPS-012 | 待补充 | 已覆盖 | 1.2、2.6、7.3、7.5、14.1.16、14.1.18、14.1.21 |
...
#### 14.4.28 T31 `DATA_SERVICE` 接口口径统一回标记录（2026-02-27）

| 回标项 | 关联需求 | T31 前状态 | T31 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| `DATA_SERVICE` 主入口统一到 `applications` 单接口 | R-DATA-010、R-OPS-012 | 待补充 | 已覆盖 | 4.3、7.3、7.5、14.1.23 |
| `shared/embedded` 可见性与权限边界收敛（`applications` vs `embeddeddataservices`） | R-DSP-011、R-DSP-012 | 待补充 | 已覆盖 | 10.2、11.3、14.1.23 |
| 验收与覆盖回标去双口径 | R-DSP-012、R-DATA-010、R-OPS-012 | 待补充 | 已覆盖 | 14.1.2、14.1.5、14.1.23、14.4.3、14.4.28 |

#### 14.4.29 T32 跨边界错误码词典补齐回标记录（2026-02-27）

| 回标项 | 关联需求 | T32 前状态 | T32 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| `CROSS_SCOPE_REFERENCE` 纳入错误码词典并形成唯一定义 | R-ENV-004、R-OPS-004、NFR-014 | 待补充 | 已覆盖 | 7.2、11.2、13.4、14.1.24 |
...
#### 14.4.31 T34 执行底座部署视图一致性修复回标记录（2026-02-27）

| 回标项 | 关联需求 | T34 前状态 | T34 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| 部署视图补齐 `Redis` 控制面组件与连接关系 | R-OPS-005、R-OPS-009 | 待补充 | 已覆盖 | 3.4、14.1.26 |
| 逻辑架构、部署视图、执行编排三处执行底座口径收敛 | R-OPS-005、R-OPS-009、NFR-003 | 待补充 | 已覆盖 | 3.2、3.4、8.1.1、14.1.26 |
| 验收/回标/变更治理联动闭环 | R-OPS-010 | 待补充 | 已覆盖 | 14.1.26、14.4.31、15.2.2（第 30 条） |

#### 14.4.32 T35 “用户”术语去歧义与角色语义统一回标记录（2026-02-27）

| 回标项 | 关联需求 | T35 前状态 | T35 后状态 | 证据章节 |
| --- | --- | --- | --- | --- |
| `user` 角色词义冻结（禁止与泛指主体混用） | R-ENV-004、R-OPS-004 | 待补充 | 已覆盖 | 2.5、10.2、11.3、14.1.27 |
| 数据服务命名文案去歧义（`用户名称` -> `调用方提交名称`） | R-DSP-011、R-DSP-012 | 待补充 | 已覆盖 | 4.3、9.4、14.1.27 |
| 验收与覆盖回标术语一致性闭环 | R-ENV-004、R-OPS-004、R-DSP-011、R-DSP-012 | 待补充 | 已覆盖 | 14.1.13、14.1.27、14.4.3、14.4.32 |
...
19. Redis 执行底座收敛复核：凡涉及 `R-OPS-005/R-OPS-007/R-OPS-009` 的执行链路变更，必须同步核对
    ...
    并确认 `15.3` 无未闭环 OQ。
20. 网关配置口径与术语一致性复核：凡涉及 `R-GTW-006` 或网关配置语义改词，必须同步核对
    `4.6`（正文边界）、`7.3/7.6`（接口与事件契约）、`14.1.15/14.4.20`（验收与回标）、
    `15.3.3`（开放项证据），并执行全文术语扫描，禁止旧术语回流。
...
24. 记忆体资源口径一致性复核：凡涉及 `R-MEM-001~008` 或 `R-DSP-010~013` 的变更，必须同步核对
    `4.3`（`mem0` 迁移与 embedded 标记）、`4.9/5.2/5.3/5.4`（Memory 模型与关系）、
    `7.4/7.5`（`MEMORY` 查询分支与 OpenAPI）、`8.10`（级联补偿）、
    `10.2/11.3`（权限与隔离）、`12.2/13.1`（容量与性能指标）、
    `14.1.19/14.4.24`（验收与回标），禁止将记忆体能力回退为数据服务组件子类型。
25. 需求编号迁移一致性复核：凡涉及 `R-DSP/R-DATA` 编号重排或语义迁移，必须同步核对
    `14.1`（验收矩阵）、`14.4.3`（逐条覆盖）、`14.4.25`（迁移表与回标）、
    `15.1`（交付物）与本流程门禁；未完成“正文 + 验收 + 回标”三处同步前，禁止标记任务完成并进入实现拆分。
26. 全文一致性冲突复核：凡涉及 `R-ENV-005`、`R-DATA-010/011`、`R-OPS-002/004/012/013` 的口径调整，必须同步核对
    `1.2/2.6/4.1`（术语与时态）、`7.1/7.3/7.5`（路径/作用域/OpenAPI）、
    `10.3`（Secret 幂等）、`14.1.16/14.1.18/14.1.21` 与 `14.4.21/14.4.26`（历史/现行隔离与回标），
    并完成未定义术语扫描，禁止回引 `switch` 或恢复 `/instances` 现行主路径。
27. `DATA_SERVICE` 接口边界复核：凡涉及 `R-DSP-011/012`、`R-DATA-010`、`R-OPS-012` 的变更，必须同步核对
    `4.3`（接口边界）、`7.3/7.5`（canonical path 与 OpenAPI）、`10.2/11.3`（可见性与权限）、
    `14.1.23/14.4.28`（验收与回标），禁止恢复 `applications` 与 `dataservices` 双主口径。
...
30. 执行底座部署视图一致性复核：凡涉及 `R-OPS-005/R-OPS-009/NFR-003` 的执行底座或部署清单变更，必须同步核对
    `3.2`（逻辑架构）、`3.4`（部署视图）、`8.1.1`（队列/锁/EventBus 语义）、
    `14.1.26/14.4.31`（验收与回标），禁止出现“逻辑架构声明与部署视图缺项”断层。
...
| T13 非阻断开放项收敛（网关配置口径/下载鉴权） | 通过 | 已完成 4.6、7.3、7.5、14.1.6、14.4.10 与 15.3 关单回写，OQ-02/OQ-03 关闭 |
...
| T34 执行底座部署视图一致性修复（补齐 Redis） | 通过 | 已完成 3.2、3.4、8.1.1、14.1.26、14.4.31、15.2 与 ADR 回写 |
| T35 “用户”术语去歧义与角色语义统一 | 通过 | 已完成 2.5、4.3、9.4、10.2、11.3、14.1.27、14.4.3、14.4.32、15.1 与 ADR 回写 |
...
#### 15.3.3 已关闭问题（T9/T13/T19/T20/T22/T23）

| 问题 ID | 关闭日期 | 关闭结论 | 回写章节 |
| --- | --- | --- | --- |
| OQ-01 | 2026-02-23 | `allow_multi_instance` 默认值定稿为 `false`，由 `super_admin/workspace_admin` 在授权 workspace 配置，变更仅影响新建实例 | 4.4、14.4.6、`docs/decision.md`（ADR-086） |
| OQ-02 | 2026-02-23 | 网关配置变更与回滚审计口径定稿（历史记录，后续由 T23 收敛正文），`TC-GTW-01-A` 可执行 | 4.6、14.1.6、14.4.10、`docs/decision.md`（ADR-090） |
...
| OQ-07 | 2026-02-27 | T23 完成网关配置口径与术语一致性收敛，`TC-GTW-02-*` 可执行 | 4.6、7.3、7.6、14.1.15、14.4.20、15.2、ADR-099 |

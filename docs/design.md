...
1. 建立 `FR/NFR/AC -> 设计章节 -> TC` 的可追踪矩阵，保证验收条目可定位到具体设计锚点。
2. 建立 `TC-001~TC-022 -> 设计锚点` 的反向矩阵，确保不存在“无设计依据测试”。
3. 以 requirements §13.2（GC-07/GC-08/GC-09）为门禁，对 Mermaid 合规性与“可直接指导编码”粒度做最终检查。

判定口径：

1. 设计锚点最小粒度为“章节小节/规则 ID/表格名称”。
2. 每个 AC 必须至少映射到一个 TC 与一个设计章节。
3. 每个 TC 必须至少映射到一个设计锚点；若单个 TC 覆盖多需求，按“组合覆盖”计入闭环。

### 10.2 FR/NFR/AC -> 设计章节 -> TC 追踪矩阵

| 追踪ID | 设计章节锚点 | 对应 TC | 验证证据 |
| --- | --- | --- | --- |
| FR-ACL-001, FR-ACL-002, FR-ACL-003 | §2.3 角色-动作-资源矩阵、§2.5 错误码模板、§3.5 错误码体系 | TC-004 | 非授权角色调用 CUD 时统一返回 `AETHER_PERMISSION_DENIED`。 |
| FR-ACL-004 | §2.3 同工作空间内授权规则 #1 | TC-005 | 规则明确“不按创建者做二级隔离”。 |
| FR-WS-001 | §2.4 SCOPE-01~SCOPE-05、§2.6 作用域时序 | TC-017 | QueryTask 与 CUD 均在工作空间维度执行同一作用域判定。 |
| FR-WS-002 | §2.2 `WorkspaceClusterBinding.registry_refs[]`、§2.4 SCOPE-06 | TC-012 | 仓库关联边界定义为“上层维护、Aether 仅消费引用”。 |
| FR-WS-003 | §2.4 SCOPE-02、§2.5 缺失 `cluster_id` 错误模板、§3.4 字段校验 | TC-001 | Create 缺失 `cluster_id` 返回 `AETHER_INVALID_ARGUMENT`。 |
| FR-RT-001 | §5.3 六类资源注册配置表、§5.5 生命周期映射 | TC-012 | 六类资源均通过同一 CUD/Query 路径。 |
| FR-RT-002, FR-RT-003, FR-RT-004, FR-RT-005 | §5.3 资源类型注册项、§5.4 模板来源边界 | TC-012 | 数据服务/低代码/DevBox/网关均以统一注册模型接入。 |
| FR-RT-006, FR-RT-007 | §5.3 `HIGH_CODE_APP/MEMORY` 注册规则、§6.3 依赖缺失处理 | TC-008, TC-012 | 高代码与记忆体在依赖解析与统一生命周期中可验证。 |
| FR-CORE-001 | §5.5 生命周期参数与统一执行器映射 | TC-012 | 执行入口对资源类型无分叉。 |
| FR-CORE-002 | §5.6 新类型接入模板、§5.8 扩展性约束 | TC-013 | 新增类型仅改注册与规则，不改通用执行编排核心。 |
| FR-CORE-003 | §3.3 `ResourceRef` 与 CUD 响应字段约束、§3.6 映射 | TC-002 | CUD 受理响应包含稳定资源标识与任务标识。 |
| FR-CORE-004 | §7.3 渲染规则链、§7.4 双输出契约 | TC-019, TC-022 | 渲染输出支持数据/文件两形态且保持确定性。 |
| FR-API-001 | §3.2 代码级接口签名、§3.6 OpenAPI 到代码接口映射 | TC-002, TC-003, TC-019 | CUD/Query/Render 均收敛到代码接口，不扩展额外对外协议。 |
| FR-API-002 | §3.3 `CUDRequest`、§3.4 必填与枚举校验 | TC-001 | 缺失必填字段直接参数错误。 |
| FR-API-003 | §3.3 `CUDResponse`、§4.3 状态机起始态 | TC-002 | 合法 Create 同步返回 `ACCEPTED + task_id`。 |
| FR-API-004 | §3.2 `QueryTask` 签名、§3.6 Query 映射、§4.5 Query 返回策略 | TC-003 | Query 仅支持 `task_id` 单任务查询。 |
| FR-API-005 | §3.5 错误码总表、§7.6 渲染失败分支、§8.6 查询失败映射 | TC-004, TC-011, TC-021 | 参数/权限/渲染/查询错误均可落到明确错误类别。 |
| FR-API-006 | §7.4 `RenderValues` 输出契约 | TC-019 | 渲染接口与 CUD 输入语义同构。 |
| FR-EXEC-001 | §4.3 状态流转、§4.6 状态机视角接口映射 | TC-002 | CUD 触发执行并进入异步任务状态机。 |
| FR-EXEC-002 | §8.2 无状态组件模型、§8.4 执行路径（无持久化） | TC-015 | 执行链路遵循 Go + Gin + Helm 与无状态实现边界。 |
| FR-EXEC-003 | §4.4 幂等命中/冲突规则、§4.8 并发约束 | TC-006, TC-007 | 同幂等键同指纹命中，异指纹冲突。 |
| FR-EXEC-004 | §7.5 Helm 等效规则、§7.8 执行时序 | TC-020 | Create/Update 与手工 Helm 命令保持语义等效。 |
| FR-Q-001 | §4.5 Query 降级规则、§8.5 最小可信响应 | TC-011 | 上下文不可达时区分 `UNKNOWN` 与查询错误。 |
| FR-Q-002 | §3.2 `QueryTask` 契约、§4.5 单任务状态结果语义 | TC-003 | 返回当前任务状态与结果，不引入列表查询。 |
| FR-DM-001 | §8.2 无状态模型、§8.4 无持久化执行路径 | TC-011 | 查询/执行不依赖资源表、任务表强制落库。 |
| FR-DM-002 | §6.2 `spec.dependencies` schema、§6.3 缺失处理 | TC-008 | `required=true` 缺失依赖返回 `AETHER_DEPENDENCY_MISSING`。 |
| FR-DM-003 | §6.5 查询边界、§6.9 禁止操作 #4 | TC-018 | Aether 不输出依赖展示语义模型。 |
| FR-DM-004 | §6.4 删除策略定义、§6.5 删除映射 | TC-009, TC-010 | `RETAIN` 与 `CASCADE_EXPLICIT` 行为由显式参数驱动。 |
| NFR-001 | §8.1 需求追踪、§8.2 运行时模型 | TC-015 | 技术栈边界固定为 Go + Gin + Helm。 |
| NFR-002 | §5.6 接入模板、§5.8 扩展性约束 | TC-013 | 扩展新类型不侵入通用执行器。 |
| NFR-003 | §4.4 幂等/冲突判定、§4.8 并发禁止操作 | TC-006, TC-007 | 重试与冲突语义稳定，可支撑可靠性目标。 |
| NFR-004 | §9.5 日志事件与字段字典、§9.7 关联约束 | TC-016 | 日志包含操作者、请求摘要、执行结果。 |
| NFR-005 | §7.3 确定性约束、§7.7 一致性约束、§7.6 可追溯字段 | TC-022 | 同请求同模板版本重复渲染语义一致且可追溯。 |
| FR-INT-001 | §9.3 OpenAPI 映射、§9.4 双路径调用规范、§9.6 失败分支 | TC-014 | 上层完成前置校验后调用 Aether，职责边界清晰。 |
| AC-001 | §5.3 六类资源配置、§5.5 统一执行器、§2.3 QueryTask 权限一致性 | TC-012, TC-017 | 六类资源在统一接口与统一权限边界下完成生命周期操作。 |
| AC-002 | §3.3 CUD 受理响应、§4.3 `ACCEPTED` 起始态 | TC-002 | CUD 同步受理、异步执行。 |
| AC-003 | §3.2 `QueryTask` 单查契约、§2.3 QueryTask 权限矩阵 | TC-003, TC-017 | 单任务查询与工作空间权限规则同时满足。 |
| AC-004 | §6.5 查询边界、§6.4 删除策略、§6.9 禁止操作 | TC-009, TC-018 | 依赖展示责任不上收，删除行为不引入隐式级联。 |
| AC-005 | §7.4 双输出形态、§7.3 渲染规则链 | TC-019 | `YAML_DATA/EXECUTABLE_FILE` 输出语义等价。 |
| AC-006 | §7.5 Helm 等效映射、§7.8 执行时序 | TC-020 | Aether Create/Update 与手工 Helm 执行结果等效。 |

### 10.3 TC-001~TC-022 设计支撑矩阵（反向校验）

| TC-ID | 设计锚点 | 支撑结论 |
| --- | --- | --- |
| TC-001 | §2.4 SCOPE-02、§2.5 错误模板、§3.4 字段校验 | 缺失 `cluster_id` 的参数错误分支已定义。 |
| TC-002 | §3.3 `CUDResponse`、§4.3 状态机起始态 | CUD 受理返回与异步执行语义完整。 |
| TC-003 | §3.2 `QueryTask`、§4.5 Query 返回策略 | 单任务查询能力与返回语义已收敛。 |
| TC-004 | §2.3 角色矩阵、§3.5 错误码映射 | 非授权 CUD 统一归类为权限拒绝。 |
| TC-005 | §2.3 同工作空间授权规则 #1 | 同空间不按创建者隔离规则可直接验证。 |
| TC-006 | §4.4 幂等命中规则、§4.8 并发约束 | 同幂等键同指纹命中返回同结果。 |
| TC-007 | §4.4 冲突规则、§3.5 `AETHER_CONFLICT` | 同幂等键异指纹冲突分支已定义。 |
| TC-008 | §6.2 依赖字段 schema、§6.3 缺失处理、§6.6 错误映射 | 强依赖缺失失败路径完整。 |
| TC-009 | §6.4 `RETAIN` 行为定义、§6.5 删除映射 | 主资源删除不强制处理依赖。 |
| TC-010 | §6.4 `CASCADE_EXPLICIT` 行为、§6.7 并发一致性约束 | 仅删除显式列出的依赖项。 |
| TC-011 | §4.5 Query 降级、§8.5 最小可信响应、§8.6 错误码映射 | `UNKNOWN` 与 `AETHER_QUERY_CONTEXT_UNAVAILABLE` 分界明确。 |
| TC-012 | §5.3 六类资源注册、§5.5 生命周期映射、§2.3 权限矩阵 | 六类资源共享统一 CUD/Query 主路径。 |
| TC-013 | §5.6 接入模板、§5.8 扩展约束 | 新类型扩展无需改通用执行核心。 |
| TC-014 | §9.2 前置校验责任、§9.4 双路径规范、§9.6 失败分支 | 上层前置校验与 Aether 边界可直接执行。 |
| TC-015 | §8.1 NFR-001 追踪、§8.2 组件与运行时模型 | 技术栈约束在架构层有显式锚点。 |
| TC-016 | §9.5 日志字段字典、§9.7 审计关联约束 | 日志字段满足可审计与问题追踪要求。 |
| TC-017 | §2.3 QueryTask 与 CUD 同矩阵、§5.3 六类资源配置 | 六类资源 QueryTask 权限边界一致。 |
| TC-018 | §6.5 查询边界声明、§6.9 禁止操作 #4 | Query 不承载依赖展示语义。 |
| TC-019 | §7.3 渲染规则链、§7.4 双输出契约、§7.8 时序 | 渲染双输出语义等价且可执行。 |
| TC-020 | §7.5 Helm 等效规则、§7.8 时序 | Aether 执行语义与 Helm 标准命令对齐。 |
| TC-021 | §7.6 渲染失败分支、§3.5 错误码总表 | 渲染失败统一映射到“渲染错误”类别。 |
| TC-022 | §7.3 确定性约束、§7.7 一致性约束、§7.6 可追溯日志 | 重复渲染一致性与执行输入追溯链完整。 |

反向校验结论：`TC-001~TC-022` 均已映射到至少一个设计锚点，不存在“无设计依据测试”。

### 10.4 Mermaid 合规性检查清单（GC-08）

| 检查项 | 结果 | 证据 |
| --- | --- | --- |
| 全文图示语法仅使用 Mermaid | 通过 | `docs/design.md` 全文 Mermaid 代码块共 16 处；未出现 PlantUML 代码块与图片标签语法。 |
| 架构/状态/时序类关键图均使用 Mermaid | 通过 | §1.5、§2.6、§3.8、§4.7、§5.7、§6.8、§7.8、§8.8、§9.8、§10.6 均为 Mermaid 代码块。 |
| T10 收口流程图遵循 Mermaid 输出 | 通过 | §10.6 使用 Mermaid `sequenceDiagram` 描述收口流程。 |

### 10.5 设计完成度门禁清单（GC-09）

| 门禁项 | 覆盖章节 | 结果 |
| --- | --- | --- |
| 数据结构 | §2.2、§3.3、§4.2、§5.2、§6.2、§7.2、§9.5.3 | 通过 |
| 状态流转 | §4.3、§4.5、§6.4 | 通过 |
| 接口契约 | §3.2、§3.6、§6.5、§7.4、§9.3 | 通过 |
| 错误码与失败分支 | §3.5、§6.6、§7.6、§8.6、§9.6 | 通过 |
| 并发/幂等 | §4.4、§4.8、§6.7、§7.7、§8.7、§9.7 | 通过 |
| 关键流程时序 | §1.5、§2.6、§3.8、§4.7、§5.7、§6.8、§7.8、§8.8、§9.8、§10.6 | 通过 |
| 边界条件与禁止操作 | §3.9、§4.8、§5.9、§6.9、§7.9、§8.9、§9.9 | 通过 |

门禁结论：当前 `docs/design.md` 已满足“可直接指导编码”最小覆盖项。

### 10.6 验收收口执行时序（Mermaid）

```mermaid
sequenceDiagram
    participant Req as Requirements Baseline
    participant Design as Design Owner
    participant QA as Test Owner
    participant Gate as Design Gate

    Req->>Design: 输入 FR/NFR/AC + TC-001~TC-022
    Design->>Design: 建立需求正向矩阵(10.2)
    Design->>Design: 建立 TC 反向矩阵(10.3)
    QA->>Design: 逐条核对 TC 是否存在设计锚点
    alt 存在未映射 TC 或关键门禁缺失
        Design->>Design: 回填缺失章节并更新矩阵
        Design->>QA: 重新评审
    else 全量通过
        Design->>Gate: 提交 Mermaid/完成度门禁结果
        Gate-->>Design: 通过并冻结收口基线
    end
```
...
| T10 DoD | 设计落点 |
| --- | --- |
| AC 条目全部映射到具体设计章节与至少一个 TC | §10.2 中 AC-001~AC-006 全量映射 |
| `TC-001~TC-022` 全部有对应设计支撑点，不存在“无设计依据测试” | §10.3 反向矩阵 + 反向校验结论 |
| 通过“可直接指导编码”检查，不留关键空白章节 | §10.5 门禁清单（GC-09）全部通过 |

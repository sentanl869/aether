...
- 超级管理员：
  - 创建/管理工作空间（平台管理面能力，Aether 仅消费结果）。
  - 关联工作空间与纳管集群、镜像仓库（平台管理面能力，Aether 仅消费结果）。
...
- R-ENV-002：工作空间创建及其与纳管集群、镜像仓库关联由平台管理面负责且仅超级管理员可执行；Aether 仅消费生效绑定用于鉴权与编排。
...
| --- | --- | --- | --- |
| 工作空间创建/删除（平台管理面） | 不适用（不属于 Aether 对外接口） | 不适用 | 不适用 |
| 集群纳管与解绑（平台管理面） | 不适用（不属于 Aether 对外接口） | 不适用 | 不适用 |
| 工作空间绑定集群/仓库（平台管理面） | 不适用（不属于 Aether 对外接口） | 不适用 | 不适用 |
...
| 嵌入式组件独立运维 | 允许（仅超管专用独立接口） | 拒绝（仅随宿主管理） | 拒绝 |
